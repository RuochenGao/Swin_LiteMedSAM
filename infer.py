...
def get_scribble_256(box, gt2D):
    ...
    # best: l/8~l/6
    try:
        bounder_shiftx = np.random.randint(int((x_max - x_min) / 8),
                                           int((x_max - x_min) / 6), (1, ))
        # bounder_shiftx = int((x_max-x_min)/6)
        bounder_shiftx = bounder_shiftx.item()
        
    except:
        bounder_shiftx = 0
    try:
        bounder_shifty = np.random.randint(int((y_max - y_min) / 8),
                                           int((y_max - y_min) / 6), (1, ))
        # bounder_shifty = int((y_max-y_min)/6
...
@torch.no_grad()
def medsam_inference(medsam_model, img_embed, fs, points_256, box_256,
                     scribble_256, new_size, original_size):
    """
    ...
    """
    box_torch = torch.as_tensor(box_256[None, None, ...],
                                dtype=torch.float,
...
def MedSAM_infer_npz_2D(img_npz_file):
    ...
    assert np.max(
        img_3c
    ) < 256, f'input data should be in range [0, 255], but got {np.unique(img_3c)}'
    H, W = img_3c.shape[:2]
    ...
    img_256_tensor = torch.tensor(img_256_padded).float().permute(
    ...
    if save_overlay and "Microscope" not in npz_name:
        ...
        for i, box in enumerate(boxes):
            ...
            show_mask((segs == i + 1).astype(np.uint8),
                      ax[1],
                      mask_color=color)

        plt.tight_layout()
        plt.savefig(join(png_save_dir,
                         npz_name.split(".")[0] + '.png'),
                    dpi=300)
        plt.close()
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    boxes_3D = npz_data[
        'boxes']  # [[x_min, y_min, z_min, x_max, y_max, z_max]]
    ...
    for idx, box3D in enumerate(boxes_3D, start=1):
        ...
        for z in range(z_middle, z_max):
            ...
            img_256 = (img_256 - img_256.min()) / np.clip(
                img_256.max() - img_256.min(), a_min=1e-8,
                a_max=None)  # normalize to [0, 1], (H, W, 3)
            ## Pad image to 256x256
            img_256 = pad_image(img_256)

            # convert the shape to (3, H, W)
            img_256_tensor = torch.tensor(img_256).float().permute(
                2, 0, 1).unsqueeze(0).to(device)
            # get the image embedding

            with torch.no_grad():
                start_time = time()
                image_embedding, fs = medsam_lite_model.image_encoder(
                    img_256_tensor)  # (1, 256, 64, 64)
                inference_time.append(time() - start_time)
            if z == z_middle:
                box_256, _ = resize_box_to_256(mid_slice_bbox_2d,
                                               original_size=(H, W))
            else:
                pre_seg = segs_3d_temp[z - 1, :, :]
                # pre_seg = segs[z-1, :, :]     # original but wrong
                if np.max(pre_seg) > 0:
...
parser.add_argument(
    ...
    type=str,
    default="workdir/Swin_LiteMedSAM.pth",
